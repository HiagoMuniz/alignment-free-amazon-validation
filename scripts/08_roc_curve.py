#!/usr/bin/env python3
"""
ROC and precision-recall curves of the proposed classifier (XGBoost with
Word2Vec, k=6) on the Amazonian data (82 viruses and 7,542 host transcripts).

Illustrative only: it uses a single embedding, and out-of-distribution
performance varies across embeddings, see embedding_variance_v2.csv. The curve
shows the threshold trade-off; it is not used to pick a threshold, which would
be biased if done on the evaluation set. The reference tools emit binary
decisions, a single point rather than a curve, so only the proposed model is
plotted.
"""
import os
...
print("training embedding and XGBoost, one illustrative run...",flush=True)
...
print(f"ROC-AUC={roc_auc:.3f}  AP(PR-AUC)={ap:.3f}  (ratio {len(pos)}:{len(neg)})")
...
a1.plot(fp/(fp+tn),tp/(tp+fn),"o",color="red",ms=9,label="threshold=0.5 (used)")
a1.set_xlabel("False positive rate (1 - specificity)"); a1.set_ylabel("Recall (sensitivity)")
a1.set_title("ROC curve"); a1.legend(loc="lower right",frameon=False); a1.grid(alpha=.3)
...
a2.set_title(f"Precision-recall curve (ratio {len(pos)}:{len(neg)})")
a2.legend(loc="upper right",frameon=False); a2.grid(alpha=.3)
fig.tight_layout()
out=E/"results/roc_pr_curve_illustrative.png"
